...
(function () {
    ...
    const ajaxErrorMsg = document.getElementById('ajax-error-msg');
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                ajaxErrorMsg.textContent = data.message || 'Login failed. Please try again.';
                ajaxError.classList.remove('hidden');
                ajaxError.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }
        } catch (err) {
            ajaxErrorMsg.textContent = 'Something went wrong. Please try again.';
            ajaxError.classList.remove('hidden');
        } finally {
            if (!document.hidden) {
