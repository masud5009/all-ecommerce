...
@section('content')
    ...
    <div class="col-lg-12">
        <div class="card">
            <x-bulk-delete :url="route('user.product.product_bulk_delete')" itemTextName="products" />
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="productType">
                                    <a class="dropdown-item"
                                        href="{{ route('user.product.create', ['type' => 'digital']) }}">
                                        {{ __('Digital Product') }}</a>
                                    <a class="dropdown-item"
                                        href="{{ route('user.product.create', ['type' => 'physical']) }}">
                                ...
                                <tbody>
                                    @foreach ($products as $key => $product)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('user.product.variant', ['product_id' => $product->id, 'language' => request()->input('language')]) }}"
                                            ...
                                            <td>
                                                @if ($product->status == 1)
                                                    <span class="badge bg-success changeStatusBtn"
                                                        ...
                                                        data-url="{{ route('user.product.status_change') }}">{{ __('Active') }}</span>
                                                @else
                                                    <span class="badge bg-danger changeStatusBtn"
                                                        data-id="{{ $product->id }}" data-value="{{ $product->status }}"
                                                        data-url="{{ route('user.product.status_change') }}">{{ __('Inactive') }}</span>
                                                @endif
                                            </td>
                                            <td>
                                                <div class="action-buttons">
                                                    ...
                                                    <form class="deleteForm d-inline-block"
                                                        action="{{ route('user.product.product_delete') }}" method="post">
                                                        @csrf
