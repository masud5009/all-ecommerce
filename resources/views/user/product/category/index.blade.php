...
@section('content')
    ...
    <div class="col-lg-12">
        <div class="card shadow">
            <x-bulk-delete :url="route('user.product.category_bulk_delete')" itemTextName="categories" />
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($category->status == 1)
                                                    <span class="badge bg-success changeStatusBtn"
                                                        ...
                                                        data-url="{{ route('user.product.category_status_change') }}">{{ __('Active') }}</span>
                                                @else
                                                    <span class="badge bg-danger changeStatusBtn"
                                                        data-id="{{ $category->id }}" data-value="{{ $category->status }}"
                                                        data-url="{{ route('user.product.category_status_change') }}">{{ __('Inactive') }}</span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="action-buttons">
                                                ...
                                                <form class="deleteForm d-inline-block"
                                                    action="{{ route('user.product.category_delete') }}" method="post">
                                                    @csrf
    ...
    @includeIf('user.product.category.create')
    @includeIf('user.product.category.edit')
@endsection
