...
@section('content')
                ...
                <aside class="hidden lg:block">
                    <div class="sticky top-24 space-y-8">
                        ...
                        <div class="rounded-2xl border border-green-100 bg-white p-6 shadow-sm">
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-900"> {{ __('Categories') }}
                            </h3>
                                ...
                                @foreach ($categories as $category)
                                    <li>
                                        <a href="{{ route('frontend.shop', array_merge($filters, ['category' => $category->id])) }}"
                                            ...
                                            {{ $category->name }}
                                            <span
                                                class="text-xs text-slate-400">{{ $category->productContent->count() }}</span>
                                        </a>
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            ...
                            <p class="text-sm text-slate-600">
                                {{ __('Showing') }} <span
                                    class="font-medium text-slate-900">{{ $products->count() }}</span> of
                                <span class="font-medium text-slate-900">{{ $products->total() }}</span>
                                {{ __('products') }}
                            </p>
                        </div>
                    ...
                    @else
                        ...
                        <div class="flex flex-col items-center justify-center py-20 text-center">
                            <div class="flex h-24 w-24 items-center justify-center rounded-full bg-green-100">
                                <svg class="h-12 w-12 text-green-600" viewBox="0 0 24 24" fill="none"
    ...
    <div id="mobile-filter-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute bottom-0 left-0 right-0 max-h-[85vh] overflow-y-auto rounded-t-3xl bg-white p-6">
            ...
            <!-- Categories -->
            <div class="mb-6">
                <h4 class="text-sm font-semibold uppercase tracking-wide text-slate-900 mb-3"> {{ __('Categories') }}</h4>
                <div class="space-y-2">
                    <a href="{{ route('frontend.shop', array_merge($filters, ['category' => null])) }}"
            ...
            <!-- Price Filter -->
            <div class="mb-6">
                <h4 class="text-sm font-semibold uppercase tracking-wide text-slate-900 mb-3"> {{ __('Price Range') }}
                </h4>
