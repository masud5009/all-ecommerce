    ...
    <div class="grid gap-4 lg:grid-cols-[300px_1fr]">
        <div class="rounded-2xl border border-green-100 bg-green-50/40 p-5">
            ...
            <div class="mt-2 flex items-center gap-1">
                @for ($i = 1; $i <= 5; $i++)
                    <svg class="h-4 w-4 {{ $i <= floor($productRating) ? 'text-amber-400' : 'text-slate-300' }}" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d="M12 17.3l-6.2 3.7 1.7-7.1L2 9.2l7.3-.6L12 2l2.7 6.6 7.3.6-5.5 4.7 1.7 7.1L12 17.3Z" />
                    </svg>
                @endfor
            </div>
            <p class="mt-2 text-sm text-slate-600">Based on {{ $productReviews }} {{ \Illuminate\Support\Str::plural('review', $productReviews) }}</p>
        </div>
            ...
            @for ($star = 5; $star >= 1; $star--)
                ...
                <div class="flex items-center gap-3 {{ $star > 1 ? 'mb-2' : '' }}">
                    ...
                    <div class="h-2.5 flex-1 overflow-hidden rounded-full bg-slate-100">
                        <div class="h-full rounded-full bg-amber-400" style="width: {{ number_format($percent, 2, '.', '') }}%"></div>
                    </div>
    ...
    @if (count($productReviewList) > 0)
        <div class="grid gap-3 sm:grid-cols-2">
            @foreach ($productReviewList as $review)
                <div class="rounded-2xl border border-green-100 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <span class="flex h-10 w-10 items-center justify-center rounded-full bg-green-100 text-sm font-semibold text-green-700">
