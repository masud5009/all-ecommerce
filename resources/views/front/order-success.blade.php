...
@section('content')
    <div class="bg-gradient-to-b from-green-50/80 via-white to-white min-h-screen">
        <div class="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
            ...
            <div class="text-center">
                <div class="relative mx-auto mb-6 inline-flex">
                    ...
                    <div class="relative rounded-full bg-green-100 p-6">
                        <svg class="h-16 w-16 text-green-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <div class="mt-4 space-y-4">
                        @foreach ($order->items as $item)
                            ...
                            <div class="flex items-center gap-4 rounded-xl bg-green-50/50 p-3">
                                <div class="h-14 w-14 flex-shrink-0 overflow-hidden rounded-lg border border-green-100 bg-white">
                                    @if ($item->product)
                                        <img src="{{ asset('assets/img/product/' . $item->product->thumbnail) }}"
                                            alt="{{ $item->product->content->first()?->name ?? 'Product' }}" class="h-full w-full object-cover">
                                    @else
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-sm font-medium text-slate-900 truncate">{{ $item->product?->content->first()?->name ?? 'Product' }}</h4>
                                    @if ($variantLabel)
                                        <p class="text-xs text-slate-500">{{ $variantLabel }}</p>
                                    @endif
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-medium text-slate-900">{{ currency_symbol($item->product_price) }} × {{ $item->qty }}</p>
                                    <p class="text-sm font-bold text-green-700">{{ currency_symbol($item->product_price * $item->qty) }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            {{-- Delivery Info --}}
            <div class="mt-10 rounded-2xl bg-green-50 p-6 text-center">
                ...
                <h3 class="font-semibold text-slate-900">Estimated Delivery</h3>
                <p class="mt-1 text-sm text-slate-600">Your order will be delivered within <span class="font-semibold text-green-700">2-3 business days</span></p>
            </div>
        </div>
    </div>
@endsection
