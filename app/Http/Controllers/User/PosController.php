...
use App\Models\User;
use App\Models\User\Product as UserProduct;
...
class PosController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['tables'] = Table::where('user_id', $user_id)
            ->orderBy('serial_number', 'desc')
            ->get();

        $data['categories'] = ProductCategory::where('user_id', $user_id)
            ->where('status', 1)
            ->where('language_id', $language->id)
            ->orderBy('serial_number', 'desc')
            ->get();

        $data['products'] = UserProduct::where('user_id', $user_id)
            ->where('status', 1)
            ...
            ->map(function ($product) use ($language) {
                // Get category info from ProductCategory
                ...
                return [
                    'id' => $product->id,
                    'name' => $product->content->title ?? 'No Name',
                    'price' => $product->current_price,
                    'image' => $product->thumbnail ? asset('assets/admin/img/products/' . $product->thumbnail) : 'https://via.placeholder.com/80?text=No+Image',
                    'category' => $category->slug ?? 'uncategorized',
                    'category_name' => $category->name ?? 'Uncategorized',
                    'cooking_time' => $product->cooking_time ?? 0,
